...
def concatenate_with_micros(data, microgrid_data):
    ...
    data['process_commodity'] = data['process_commodity'][~data['process_commodity'].index.duplicated(keep='first')] # delete duplicated process commodities (for different ratios from different systems, the processes need adapted names)
