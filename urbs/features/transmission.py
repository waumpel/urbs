...
def add_transmission_dc(m, shared_lines):
    ...
    if shared_lines is not None:
        m.tra_tuples_shared = pyomo.Set(
                within=m.tra_tuples_tp, # only transport transmissions may be boundary transmissions (for now)
                initialize=shared_lines.index,
                doc='Combinations of possibel transmissions on the boundary, e.g. '
                '(South,Mid,hvac,Elec)')
        m.tra_tuples_internal = pyomo.Set(
...
def add_transmission_ac(m, shared_lines):
    ...
    m.res_transmission_input_by_apparent_power_lower = pyomo.Constraint(
        m.tm, m.tra_tuples_ac,
        rule=def_transmission_input_by_apparent_power_rule_lower,
        doc='-transmission.cap-up <= transmission.flow(active)')

    m.res_transmission_input_by_apparent_power_upper = pyomo.Constraint(
        m.tm, m.tra_tuples_ac,
        rule=def_transmission_input_by_apparent_power_rule_upper,
        doc='transmission.flow(active) <= transmission.cap-up')
...
def def_transmission_input_by_apparent_power_rule_lower(m, tm, stf, sin, sout, tra, com):
    return (-m.dt * m.cap_tra[stf, sin, sout, tra, com] <=
            m.e_tra_in[tm, stf, sin, sout, tra, 'electricity'])

def def_transmission_input_by_apparent_power_rule_upper(m, tm, stf, sin, sout, tra, com):
        ...
        if m.mode['dcpf']:
            return (
                sum(m.e_tra_in[(tm,) + t] *
                    ...
                    m.transmission_dict['cost_factor'][t]
                    for tm in m.tm
                    for t in (m.tra_tuples_tp | m.tra_tuples_dc) & m.tra_tuples_internal)
                + 0.5 * sum(
                    m.e_tra_in[(tm,) + t] *
                    m.weight *
                    m.typeperiod['weight_typeperiod'][(m.stf_list[0],tm)] *
                    ...
                    m.transmission_dict['cost_factor'][t]
                    for tm in m.tm
                    for t in (m.tra_tuples_tp | m.tra_tuples_dc) & m.tra_tuples_shared)
            )
        if m.mode['acpf']:
            return (
                sum(m.e_tra_in[(tm,) + t] *
                    ...
                    m.transmission_dict['cost_factor'][t]
                    for tm in m.tm
                    for t in (m.tra_tuples_tp | m.tra_tuples_ac_dc) & m.tra_tuples_internal)
                + 0.5 * sum(
                    m.e_tra_in[(tm,) + t] *
                    m.weight *
                    m.typeperiod['weight_typeperiod'][(m.stf_list[0],tm)] *
                    ...
                    m.transmission_dict['cost_factor'][t]
                    for tm in m.tm
                    for t in (m.tra_tuples_tp | m.tra_tuples_ac_dc) & m.tra_tuples_shared)
            )
        else:
            return (
                sum(m.e_tra_in[(tm,) + t] *
